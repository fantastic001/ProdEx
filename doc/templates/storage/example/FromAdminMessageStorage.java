...
public class FromAdminMessageStorage 
{
	private ArrayList<FromAdminMessage> load() 
	{
		...
		try 
		{
			Reader reader = new FileReader(StorageManager.getInstance().getJSONPath("FromAdminMessage"));
	...
	private boolean save(ArrayList<FromAdminMessage> data) 
	{
		Gson gson = new Gson();
		try {
		    FileWriter writer = new FileWriter(StorageManager.getInstance().getJSONPath("FromAdminMessage"));
