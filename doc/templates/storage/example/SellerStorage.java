...
public class SellerStorage 
{
	private ArrayList<Seller> load() 
	{
		...
		try 
		{
			Reader reader = new FileReader(StorageManager.getInstance().getJSONPath("Seller"));
	...
	private boolean save(ArrayList<Seller> data) 
	{
		Gson gson = new Gson();
		try {
		    FileWriter writer = new FileWriter(StorageManager.getInstance().getJSONPath("Seller"));
